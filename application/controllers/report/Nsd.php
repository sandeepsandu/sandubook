<?php
	...
	class Nsd extends MY_Controller {
		...
		public function index(){
            $data['suplr'] =  $this->nsd_model->get_all_suppliers();
...
//                }
               ...
               // $data['suplrid'] = $this->input->post('suplr_name');
                ...
                $data['nsdlist'] =  $this->nsd_model->get_search_nsd($fromdate,$todate);
            }
            else
            {
                $data['datefrom']="";
               ...
               // $data['dateto']=date('m/d/Y ');
                $data['nsdlist'] =  $this->nsd_model->get_all_nsd();
            }
			$data['view'] = 'admin/report/nsd_list';
			$this->load->view('admin/layout', $data);
		}
        public function purchase_list(){
            $data['purch'] =  $this->nsd_model->get_all_purchase();
		...
		public function add(){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$result = $this->nsd_model->add_purchase($data);
					if($result){
		...
		public function edit($id = 0){
			...
			if($this->input->post('submit')){
                ...
                $this->form_validation->set_rules('amount', 'Amount', 'trim|required');
				if ($this->form_validation->run() == FALSE) {
					$data['user'] = $this->nsd_model->get_purchase_by_id($id);
					...
					$data = $this->security->xss_clean($data);
					$result = $this->nsd_model->edit_purchase($data, $id);
					if($result){
