...
//                }
               ...
               // $data['suplrid'] = $this->input->post('suplr_name');
                ...
                $data['ledgerlist'] =  $this->ledger_model->get_search_ledgers($fromdate,$todate);
            }
            else
            {
                $data['datefrom']="";
			...
			$data['view'] = 'admin/report/ledger_list';
			$this->load->view('admin/layout', $data);
		}
        public function purchase_list(){
            $data['purch'] =  $this->ledger_model->get_all_purchase();
            $data['view'] = 'admin/purchase/purchase_list';
            $this->load->view('admin/layout', $data);
        }
		public function add(){
            $data['suplr'] =  $this->ledger_model->get_all_suppliers();
            $data['godown'] =  $this->ledger_model->get_all_godowns();
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$result = $this->ledger_model->add_purchase($data);
					if($result){
		...
		public function edit($id = 0){
			...
			if($this->input->post('submit')){
                ...
                $this->form_validation->set_rules('amount', 'Amount', 'trim|required');
				if ($this->form_validation->run() == FALSE) {
					$data['user'] = $this->ledger_model->get_purchase_by_id($id);
					...
					$data = $this->security->xss_clean($data);
					$result = $this->ledger_model->edit_purchase($data, $id);
					if($result){
